"""
export_fixed_camera_slope_check.py

Renders with a FIXED, non-tracking camera (locked in world space, does
NOT follow the robot's position or orientation) specifically to settle
the uphill/downhill labeling question unambiguously -- a tracking camera
that inherits the robot's own pitch can make a genuinely-tilted floor
look level, which is suspected to have caused confusion earlier tonight.
"""
import sys
import numpy as np
import mujoco as mj
...
slope_deg = float(sys.argv[1]) if len(sys.argv) > 1 else 2.0
out_name = sys.argv[2] if len(sys.argv) > 2 else f"fixed_cam_slope{slope_deg:.0f}.mp4"
...
# Informed controller (true slope given directly) so the robot actually
# tracks the terrain correctly -- we want to see the TERRAIN tilt clearly,
# not a struggling/falling robot
...
print(f"Running slope_deg={slope_deg} (informed) for {RUN_LENGTH_S}s...")
...
print(f"Rendering {len(q_log)} frames with a FIXED, non-tracking camera...")

renderer = mj.Renderer(mujoco_go2.model, height=720, width=1280)
data_replay = mj.MjData(mujoco_go2.model)

# FIXED camera: fully manual, no tracking of any body at all. Positioned
# off to the side and slightly elevated, looking at the origin -- a
# world-frame view where any real terrain tilt will be plainly visible
# as a diagonal floor line, regardless of the robot's own orientation.
cam = mj.MjvCamera()
cam.type = mj.mjtCamera.mjCAMERA_FREE
cam.lookat = np.array([2.0, 0.0, 0.0])
cam.distance = 6.0
cam.elevation = -10
cam.azimuth = 90
