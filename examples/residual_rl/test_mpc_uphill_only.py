"""
test_mpc_uphill_only.py

Larger, dedicated test of pure MPC+WBC (NO residual policy involved at
all) specifically on UPHILL slope, across a range of angles -- tightens
the n=9 sample from the earlier channel ablation with a proper n=50+ run.

Positive slope_deg = uphill, matching the sign convention already used
throughout this project (confirmed via mpc_residual_env.py's
_sample_terrain and the channel-ablation script's direction inference).
"""
import sys
import numpy as np
import mujoco as mj
...
if __name__ == "__main__":
    ...
    slope_angles = [2.0, 5.0, 8.0, 11.0, 15.0]  # uphill only (positive)

    total_survived, total_trials = 0, 0
    for slope_deg in slope_angles:
        results = [run_trial(slope_deg) for _ in range(n_per_angle)]
        survived = sum(1 for r in results if not r["fell"])
        total_survived += survived
        total_trials += n_per_angle
        for i, r in enumerate(results):
            print(f"slope={slope_deg:+.1f}deg trial {i}: fell={r['fell']} "
                  f"reason={r['reason']} steps={r['steps']}")
        print(f"=== slope={slope_deg:+.1f}deg: {survived}/{n_per_angle} survived ===\n")

    print(f"\n=== UPHILL OVERALL: {total_survived}/{total_trials} "
          f"({total_survived/total_trials:.1%}) survived across "
          f"{len(slope_angles)} angles, n={n_per_angle} each ===")
