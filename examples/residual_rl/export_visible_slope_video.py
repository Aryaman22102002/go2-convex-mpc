"""
export_visible_slope_video.py

Uses the spare 'platform' box geom as the ACTUAL walking surface (large,
tilted, real collision) instead of the plane (which renders its tilt
incorrectly in MuJoCo, per tonight's investigation, despite being
physically correct). This gives a real video with both correct physics
AND a visibly tilted surface.
"""
import sys
import numpy as np
import mujoco as mj
...
out_name = sys.argv[2] if len(sys.argv) > 2 else f"visible_slope_{slope_deg:.0f}.mp4"

model = get_universal_model()
set_slope(model, slope_deg)  # tilts the (invisible-rendering) floor -- still
                              # governs physics if the box isn't covering it,
                              # but we override collision with the box below

floor_id = mj.mj_name2id(model, mj.mjtObj.mjOBJ_GEOM, "floor")
platform_id = mj.mj_name2id(model, mj.mjtObj.mjOBJ_GEOM, "platform")

# Make the box the REAL walking surface: same tilt as the floor, large,
# real collision. Disable the floor's own collision so the robot only
# ever touches the (visibly tilted) box.
...
model.geom_friction[platform_id] = [0.8, 0.005, 0.0001]
...
print(f"Running slope_deg={slope_deg} (informed, box surface) for {RUN_LENGTH_S}s...")
